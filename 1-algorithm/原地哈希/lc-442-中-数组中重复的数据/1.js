...
var findDuplicates = function (nums) {
  ...
  for (let i = 0; i < nums.length; ) {
    if (nums[i] !== nums[nums[i] - 1]) {
      ...
      // console.log(`for loop swaped ${i} result: ${JSON.stringify(nums)}`);
      // 交换后，“nums[nums[i] - 1]”（原来的“nums[i]”） 符合了条件， 但 nums[i]，也就是原来的“nums[nums[i] - 1]”
      // 所以需要再来一次
    } else {
      i++;
      // console.log(`for loop SAME ${i} result: ${JSON.stringify(nums)}`);
    }
  }
