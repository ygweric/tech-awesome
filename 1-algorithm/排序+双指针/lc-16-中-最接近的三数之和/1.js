...
var threeSumClosest = function(nums, target) {
  ...
  // curLeft 已经被使用过
  let getMinDiffSum =(curLeft, curCount, curSum, values, indexes)=>{
    recursiveCount++
    if (curCount === 3) {
      ...
      // console.log('value',values, 'indexes',indexes, 'minDiff: ' ,minDiff);
      // console.log('value',values,  'minDiff: ' ,minDiff);
      // console.log( 'minDiff: ' ,minDiff,'curCount', curCount, 'curLeft: ', curLeft, 'curSum: ' , curSum ,'value',values, );
      return minDiff
    }
    // console.log( 'minDiff: ' ,minDiff,'curCount', curCount, 'curLeft: ', curLeft, 'curSum: ' , curSum ,'value',values, );
    if (curCount + (nums.length - curLeft)  <3) {
      // 剪枝，剩下的数目凑不够3个了
      return minDiff
      }
    ...
    minDiff = Math.min(
      getMinDiffSum(curLeft+1, curCount+1, curSum+nums[curLeft], [...values, nums[curLeft]] , [...indexes, curLeft]),// 包括当前
      getMinDiffSum(curLeft+1, curCount, curSum, values , indexes), // 不包括当前
      )
      return minDiff
